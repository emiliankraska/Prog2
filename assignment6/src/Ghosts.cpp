...
void Ghost::move(std::vector<std::vector<int>> *map) {
    dir = findMoveDirection(map);           // Finding direction from overwritten function for every ghost
    if (isMoveColliding(map)) {             // If move collides with wall, we should just stop
        return;
    }
    Point oldPosition = Point(x, y);        // Remembering old position
    setPositionFromDir((*map)[0].size());   // Setting our position from the direction we are facing
    updatePositionOnMap(map, oldPosition);  // Updating the map position
}
